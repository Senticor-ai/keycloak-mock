...
| `KeycloakMockIntegrationTest.documentation_works` | Documentation endpoint | `test/keycloak-mock-server.test.ts` - `serves documentation as HTML and JSON` | Covered | HTTP behavior |
...
| `RedirectHelperTest.redirect_location_is_generated_correctly` | Response type and mode redirect parameters | `test/keycloak-mock-server.test.ts` - login flow and response-mode tests | Covered | Black-box HTTP behavior |
| `RedirectHelperTest.redirect_location_keeps_existing_parameters` | Preserve existing query/fragment | `test/keycloak-mock-server.test.ts` - response-mode test | Covered | Black-box HTTP behavior |
